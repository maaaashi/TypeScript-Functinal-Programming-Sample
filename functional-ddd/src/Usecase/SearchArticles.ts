...
import { tryCatch, type TaskEither, chain, fromTask } from "fp-ts/lib/TaskEither.js";
...
const searchArticleIds = (deps: Deps, cond: ValidateSearchCondition): TaskEither<SearchArticlesError, string[]> =>
  tryCatch(
    () => deps.articleIdsPort.searchArticleIds(cond),
    (e) => new SearchArticlesError("記事IDの検索中にエラーが発生しました")
  );

const findArticles = (deps: Deps, ids: string[]): TaskEither<SearchArticlesError, Articles> =>
  tryCatch(
